#!/usr/bin/env bash
# ==============================================================================
# Make_SOF_Episode_From_PNG_FLAC_Output_MP4_1080p_Video.sh
# Generates 1080p Full HD YouTube Video (1920x1080 @ 30fps) from FLAC Audio & Cover Image
...
echo -e "Resolution:     ${BOLD}${WIDTH}x${HEIGHT} (1080p Full HD @ ${FPS}fps)${NC}"

# Detect optimal Video Encoder
VCODEC="libx264"
VPRESET_ARGS=(-preset medium -crf 23)
if ffmpeg -f lavfi -i color=c=black:s=256x256 -frames:v 1 -c:v h264_nvenc -f null - >/dev/null 2>&1; then
    VCODEC="h264_nvenc"
    VPRESET_ARGS=(-preset p3 -cq 21 -g 60)
    ...
    VPRESET_ARGS=(-b:v 7500k)
    echo -e "Video Encoder:  ${BOLD}${GREEN}h264_videotoolbox (Apple Silicon / VideoToolbox Accelerated)${NC}"
else
...
echo -e "${BLUE}------------------------------------------------------------${NC}"
echo -e "${YELLOW}Rendering 1080p Full HD MP4 with FFmpeg... Please wait.${NC}\n"
...
echo -e "\n${BLUE}------------------------------------------------------------${NC}"
if [ $STATUS -eq 0 ]; then
    echo -e "${BOLD}${GREEN}✓ Successfully generated 1080p Full HD video!${NC}"
    ls -lh "$OUTPUT_FILE"
else
